...
void CSIMD128(divs)(complex128 *y, const complex128 *x, const complex128 c, const ptrdiff_t n) {
  ...
  double *my = (double *)y;
  double *mx = (double *)x;
  double norm = creal(c) * creal(c) + cimag(c) * cimag(c);
  ptrdiff_t len = 2 * n;
  __m256d YMM15 = _mm256_set_pd(creal(c)/norm, cimag(c)/norm, creal(c)/norm, cimag(c)/norm);
  __m256d YMM14 = _mm256_set_pd(-cimag(c)/norm, creal(c)/norm, -cimag(c)/norm, creal(c)/norm);
